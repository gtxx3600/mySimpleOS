#ifndef _HAHA_PROC_C_
#define _HAHA_PROC_C_
...
PUBLIC void schedule()
{
	...
	while(!greatest_ticks)
	{
		...
		if(alive==0)
		{
			next_proc=&proc_table[TASK_IDLE];
			//printf("switch to idle\n");
			//lock_notify(INTERRUPT,TASK_IDLE);
			idle_status=IDLE_RUN;
			return;
			//return;
			//disp_str("alskdjf;l");
			//disp_int(alive);
...
PRIVATE int deadlock(int src, int dest)
{
	...
	if(  ((struct proc*)(proc_table + src))->p_flags&SENDING   ){
		while (1) {
			if (p->p_flags & SENDING) {
				if (p->p_sendto == src) {
					/* print the chain */
					p = proc_table + dest;
					printl("=_=Send:%s", p->name);
					do {
						assert(p->p_msg);
	...
	if(  ((struct proc*)(proc_table + src))->p_flags&RECEIVING   ){
		p = proc_table + dest;
		while(1)
		{
			if(p->p_flags&RECEIVING){
				if(p->p_recvfrom == src) {
					p = proc_table + dest;
					printl("=_=Recieve:%s", p->name);
					do {
						//assert(p->p_msg);
						p = proc_table + p->p_recvfrom;
						printl("->%s", p->name);
					} while (p != proc_table + src);
					printl("=_=");
 ...
 *****************************************************************************/
PUBLIC void notify_int(int task_nr)
{
 ...
 * @param task_nr  The task which will be informed.
 *****************************************************************************/
PUBLIC void msg_notify(int src,int dst)
{
	struct proc* p = proc_table + dst;
	
	if ((p->p_flags & RECEIVING) && /* dest is waiting for the msg */
	    ((p->p_recvfrom == src) || (p->p_recvfrom == ANY))) {
		p->p_msg->source = src;
		p->p_msg->type = HARD_INT;
		p->p_msg = 0;
		p->has_int_msg = 0;
